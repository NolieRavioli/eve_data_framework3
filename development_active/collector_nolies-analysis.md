{public db/personal (owner) db/corperation db/alliance db} {table}
{endpoints}

all personal tables need to be character_id keyed.

i believe that the existing collectors like regions.py and structures.py need to be split between collectors and analysis.
collectors simply workat the lowest level to pass esi over to the db writer. all the analysis logic (iterating regions for regions.py and structures for structure.py) should be moved to `analysis/market_browser.py`
`analysis/market_browser.py` is analysis that uses `sde_regions` and `structures` to (its main function) build the market for the whole universe. it contains the logic to 
...
https://esi.evetech.net/characters/{character_id}
	...
	https://esi.evetech.net/characters/affiliation (bulk inset into all owner dbs. public info to enrich private database 'characters' table.)

PERSONAL agent_research
https://esi.evetech.net/characters/{character_id}/agents_research (character_id keyed)
...
CORP stats
this is just 2 fields per row 'key' 'value'
https://esi.evetech.net/corporations/{corporation_id}
alliance_id, ceo_id, creator_id, date_founded, description, faction_id, home_station_id, member_count, (member_limit), name, shares, tax_rate, ticker, every fild here is just a row each with a value.
