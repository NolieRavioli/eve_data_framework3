# tools/_ports.py
"""Abstract port interfaces for the tools framework (hexagonal / port-adapter pattern).

Concrete adapters live in _adapters.py. Tests can inject stubs via these protocols.
"""
...
class TokenPort(Protocol):
    ...
    def get(self, owner_id: int) -> dict[int, dict]:
        """Return the token map for *owner_id* as returned by util.utils.get_token."""
        ...
