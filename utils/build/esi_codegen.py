...
---------------
  ...
  core/esi/generated/__init__.py  — package marker + version check

  core/esi/generated/manifest.py  — authoritative operation catalog dict

  core/esi/generated/schemas.py   — TypedDict stubs + aliases from schemas.json

  core/esi/generated/client.py    — dynamic execute_operation() + batch helpers
...
def generate(
    ...
    Read the active ESI spec snapshot and write the ``core/esi/generated/`` package.
    ...
    output_dir:
        ...
        Where to write the package.  Defaults to ``core/esi/generated/``.
    ...
    out = Path(output_dir or "core/esi/generated")
    ...
    if not force and init_path.exists():
        ...
        if f"COMPATIBILITY_DATE = {repr(date)}" in content:
            ...
            print(f"[codegen] core/esi/generated/ already at {date} — nothing to do.  Use force=True to regenerate.")
    ...
    print(
        ...
        f"[codegen] Generated core/esi/generated/ for {date} — "
...
def check_generated_is_current() -> None:
    ...
    init_path = Path("core/esi/generated/__init__.py")

    if not init_path.exists():

        raise RuntimeError(

            "core/esi/generated/ package is missing.  Run: python build.py "
    ...
    if f"COMPATIBILITY_DATE = {repr(expected_date)}" not in content:
        ...
        raise RuntimeError(
            ...
            f"core/esi/generated/ is stale (expected {expected_date}).  "
    ...
    if expected_ops:
        ...
        from core.esi.generated import OPERATION_COUNT

        if OPERATION_COUNT != expected_ops:

            raise RuntimeError(

                f"core/esi/generated/ operation count mismatch: "
    ...
    from utils.build.domain_codegen import collectors_are_current
...
def _cli() -> None:
    ...
    parser = argparse.ArgumentParser(description="Generate core/esi/generated/ from the active ESI spec.")

    parser.add_argument("--date", metavar="YYYY-MM-DD", help="Spec compatibility date (defaults to latest)")

    parser.add_argument("--output", metavar="DIR", default="core/esi/generated", help="Output directory (default: core/esi/generated)")
