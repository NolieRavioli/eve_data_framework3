"""Internal ESI rate-limiting engine.

This module is NOT re-exported from ``core.esi`` — it is an implementation
detail of ``core.esi.request``.  External code should never import from here
directly except for ``get_esi_rate_limiter()`` (used by output.py and _api.py
for stats).
"""
...
def _effective_ttl(headers: dict, fallback_ttl: int) -> int:
    """Return the best cache TTL in seconds derived from the ``Expires`` response header."""
...
# mapped here so they appear under a named group in get_stats().
...
class EsiRateLimiter:
    ...
    def __init__(
        ...
        *,
        ...
        # Pre-seed URL→group and group→limits from routes.json static data.
        try:
            ...
            try:
                ...
                if response.status_code == 429 and attempt < self._max_retries:
                    ...
                    logger.warning(
                        ...
                        method.upper(), url, retry_after, attempt, self._max_retries,
                    )
                    retry_sleep = retry_after
                    continue
                ...
                if response.status_code == 420 and attempt < self._max_retries:
                    ...
                    logger.warning(
                        ...
                        method.upper(), url, reset_after, attempt, self._max_retries,
                    )
...
class _FairAlternatingLock:
    ...
    simultaneously they take strict alternating turns."""
    ...
    def acquire(self, lane: str) -> None:
        ...
        while True:
            ...
            self._ready[lane].wait(timeout=2.0)
