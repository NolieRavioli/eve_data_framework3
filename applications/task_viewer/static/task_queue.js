/**
 * task_queue.js — Task Queue page live updates.
 *
 * Listens for 'bus:message' events dispatched by bus.js.
 * Handles topics:
 *   - esi/rate    → ESI rate limiter card
 *   - task/events → task list live status updates
 */

// ── Cancel / clear actions ────────────────────────────────────────────────────

function cancelTask(taskId, button) {
  var root = document.getElementById('task-queue-root');
  fetch(root.dataset.urlCancel.replace('PLACEHOLDER', taskId), { method: 'POST' })
...
function clearDone() {
  var root = document.getElementById('task-queue-root');
  fetch(root.dataset.urlClear, { method: 'POST' })
    .then(function (r) { return r.json(); })
  ...
  Object.keys(groups).forEach(function (gname) {
    ...
    el.innerHTML =
      ...
      '<div style="display:flex;justify-content:space-between;font-size:0.7rem">' +
        ...
        '<span class="muted">' + (g.tokens_remaining != null ? g.tokens_remaining.toLocaleString() : '&mdash;') + ' / ' + g.tokens_limit.toLocaleString() + ' remaining</span>' +
      '</div>';
  });
}
...
document.addEventListener('bus:message', function (e) {
  ...
  if (topic === 'esi/rate') {
    applyRateStats(data);
  } else if (topic === 'task/events' && data.tasks) {
    applyTaskList(data.tasks);
  }
});
